...
export const PRICE_BOOK: PriceItem[] = [
  ...
  { key: "primer", description: "Primer", unit: "ROOM", price: 150, group: "Painting", words: "primer|prime\\b|priming|seal(?:er|ing)?\\s*coat" },
...
export function priceLinesFor(text: string, opts: PriceMatchOpts = {}): PriceLineOut[] {
  ...
  return [...merged.values()]
    ...
    .map((h) => {
      ...
      // Plaster is $6 by the owner's word (the printed sheet's $5 is an old
      // price). Scrape wording still means the repair job and never bills
      // below the wall-repair rate — and a plaster price raised by hand in
      // Settings wins over everything.
