...
export default function Settings() {
  ...
  const runSystemCheck = async () => {
    ...
    const probes: { label: string; fix: string; probe: () => Promise<boolean> }[] = [
      ...
      { label: "Email PO intake (Gmail)", fix: "add PO_INTAKE_KEY and SUPABASE_SERVICE_ROLE_KEY in Vercel → Settings → Environment Variables, then set up the Gmail script (docs/EMAIL_PO_INTAKE.md)", probe: async () => { try { const r = await fetch("/api/inbound-po"); return !!((await r.json()) as { configured?: boolean }).configured; } catch { return false; } } },
      { label: "Read-email button (Gmail web app)", fix: "in the Gmail script: Deploy → New deployment → Web app (Execute as Me, access: Anyone) → copy the URL → Vercel env GMAIL_INTAKE_URL → Redeploy (the 10-minute timer works without it)", probe: async () => { try { const r = await fetch("/api/inbound-po/run"); return !!((await r.json()) as { configured?: boolean }).configured; } catch { return false; } } },
      { label: "Smart PO reader (Claude)", fix: "add ANTHROPIC_API_KEY in Vercel → Settings → Environment Variables, then Redeploy (POs read by the rules until then)", probe: async () => { try { const r = await fetch("/api/parse-po"); return !!((await r.json()) as { smart?: boolean }).smart; } catch { return false; } } },
