...
export default function Settings() {
  ...
  const runSystemCheck = async () => {
    ...
    const probes: { label: string; fix: string; probe: () => Promise<boolean> }[] = [
      ...
      { label: "Set-up texts go out on their own", fix: "add CRON_SECRET (any long random string) and SUPABASE_SERVICE_ROLE_KEY (Supabase → Settings → API → service_role) in Vercel → Settings → Environment Variables, then Redeploy. Until then a set-up text goes out the next time somebody has the portal open. Note that on a free Vercel plan the timer only runs now and then, so a text set for a particular hour can still wait for the portal to be opened", probe: async () => { try { const r = await fetch("/api/text-due"); return !!((await r.json()) as { onItsOwn?: boolean }).onItsOwn; } catch { return false; } } },
